...
export async function initializeScene(
  ...
  try {
    ...
    const boilerModel = await loadGLB("/models/witch_cauldron.glb");

    const carrotMatrix = mat4.create();
    mat4.translate(carrotMatrix, carrotMatrix, [carrotState.x, carrotState.y, carrotState.z]);
    //mat4.scale(carrotMatrix, carrotMatrix, [0.03, 0.03, 0.03]);

    const boilerMatrix = mat4.create();
    mat4.translate(boilerMatrix, boilerMatrix, [0, 0, 0]);
    mat4.scale(boilerMatrix, boilerMatrix, [1.0, 1.0, 1.0]);

    const carrotParts = await createModelParts(gl, carrotModel, program);
    const boilerParts = await createModelParts(gl, boilerModel, program);
    ...
    sceneOptions.modelData = {
      ...
      cauldron: {
        parts: boilerParts,
        program,
        modelMatrix: boilerMatrix,
      },
    };

    if (sceneOptions.modelData.carrot) {
      globalCarrot = sceneOptions.modelData.carrot;
    } else {
      globalCarrot = null;
    }
...
canvas.dispatchEvent(new CustomEvent("sceneready"));
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const speed = 0.3; // увеличил для лучшего контроля
      switch (e.key.toLowerCase()) {
        case "w":
          carrotState.z -= speed;
          console.log("Move back", carrotState);
          sceneOptions.changed = (sceneOptions.changed == 1) ? 0 : 1;
          console.log(sceneOptions.pointSize);
          break;
        case "s":
          carrotState.z += speed;
          console.log("Move forward", carrotState);
          sceneOptions.changed = (sceneOptions.changed == 1) ? 0 : 1;
          break;
        case "a":
          carrotState.x -= speed;
    ...
    (sceneOptions as any).__keyHandler = handleKeyDown;
  } catch (e) {
...
export function renderScene(
  ...
  if (!sceneOptions.ready || !sceneOptions.modelData) {
    return;
  }

  const carrotData = sceneOptions.modelData.carrot;
  const boilerData = sceneOptions.modelData.cauldron;

  if (!carrotData || !boilerData) {
    console.warn(" ModelRenderer: Не все модели загружены");
    return;
  }
  ...
  mat4.lookAt(viewMatrix, [5, 3, 8], [0, 0, 0], [0, 1, 0]); // камера чуть дальше
  ...
  // --- Рендер: котёл ---
  mat4.multiply(modelViewMatrix, viewMatrix, boilerData.modelMatrix);
  ...
  renderModelParts(gl, program, boilerData.parts);

  // --- Обновление матрицы морковки ---
  if (carrotState.falling) {
    carrotState.speedY -= 0.01; // гравитация
    carrotState.y += carrotState.speedY;

    // Остановка при падении на дно
    if (carrotState.y < -1) {
      carrotState.y = -1;
...
export function disposeScene(gl: WebGL2RenderingContext) {
  if ((window as any).__keyHandler) {
    window.removeEventListener("keydown", (window as any).__keyHandler);
    (window as any).__keyHandler = null;
  }

  // Очистка буферов, текстур и программ
  // Пока опционально
}
