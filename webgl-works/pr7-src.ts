...
export function loadTexture(
  ...
  // Привязываем временную текстуру
  gl.bindTexture(gl.TEXTURE_2D, texture);

  // Пиксель 1x1, синий — пока изображение грузится
  gl.texImage2D(
    gl.TEXTURE_2D,
    0,
    gl.RGBA,
  ...
  image.onload = () => {
    ...
    // Генерируем мипмапы (только для степеней двойки!)
    if (isPowerOf2(image.width) && isPowerOf2(image.height)) {
      gl.generateMipmap(gl.TEXTURE_2D);
    } else {
      // Если не степень двойки — отключаем мипмапы, используем clamp
